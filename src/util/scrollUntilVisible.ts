...
function scrollUntilVisible(
    ...
    const childRelativeTop = childRect.top - parentRect.top + parentElement.scrollTop;
    if (parentElement.scrollTop > childRelativeTop - offset) {
        parentElement.scrollTop = childRelativeTop - offset;
    }

    const childBottom = childRect.bottom - parentRect.top + parentElement.scrollTop;
    const parentVisibleBottom = parentElement.scrollTop + parentElement.clientHeight;
    if (childBottom + offset > parentVisibleBottom) {
        parentElement.scrollTop = childBottom - parentElement.clientHeight + offset;
    }
}
