...
describe("useDueElementsPreview", () => {
	...
	it("Should refresh the queue when an element is created", async () => {
		// Arrange

		const initialDue: DueElementDto[] = [];
		const refreshedDue: DueElementDto[] = [
			{ elementId: { type: "card", id: "card-1" }, title: "Card 1" },
		];
		vi.mocked(getDueElements).mockResolvedValueOnce(initialDue);
		const { store } = renderWithProviders(<HookWrapper />, {
			preloadedState: { study: studyStateFor("editing") },
		});
		await waitFor(() => {
			expect(store.getState().study.queue).toEqual(initialDue);
		});
		vi.mocked(getDueElements).mockResolvedValueOnce(refreshedDue);
		...
		await waitFor(() => {
			expect(store.getState().study.queue).toEqual(refreshedDue);
		});
	});

	it("Should refresh the queue on element created even while studying", async () => {
		// Arrange
