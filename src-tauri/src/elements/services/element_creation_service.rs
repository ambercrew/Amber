...
/// Creates readings, extracts and cards. Beyond persisting the element itself, it
/// ensures each one gets a review row (extracts are reviewed like readings) so it's
/// immediately schedulable without a separate first-review step.
#[async_trait]
